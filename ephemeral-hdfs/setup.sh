...
pushd /root/spark-euca/ephemeral-hdfs
source ./setup-slave.sh

for node in $SLAVES $OTHER_MASTERS; do
  echo $node
  ssh -t -t $SSH_OPTS root@$node "/root/spark-euca/ephemeral-hdfs/setup-slave.sh" & sleep 0.3
done
wait

/root/spark-euca/copy-dir $EPHEMERAL_HDFS/conf
